...
CELLTYPE _findNameIn(const CELLTYPE va, const CELLTYPE u, const CELLTYPE caddr) { // c-addr u va -- nt | 0 ; https://forth-standard.org/proposals/find-name#contribution-58
    // Serial.print(F("XXX _find looking for:")); TXbangS(caddr, u);
...
findNameIn() { // caddr u va -- na|0; Note this ONLY currently works if caddr-- is a counted string
    SPpush(this._findNameIn(SPpop(), SPpop(), SPpop()));
}
