...
/* 0x40000080 CURRENT */ CELLL,
/* 0x40000084 undefined */ CELLL,
/* 0x40000088 CP */ 0xc,
...
/* 0x400000fc unused */ 0,
/* ==== CODE DICTIONARY ==== */
/* 0x40000100: FORTH */ F_tokenVocabulary, CELLL,
/* 0x40000108: CELLL */ F_tokenNextVal, CELLL,
...
/* 0x40000160: tokenUser */ F_tokenNextVal, CELLL,
...
/* 0x40000250: SP0 */ F_tokenUser, CELLL,
...
/* 0x40001fbc: dm+ */ F_tokenDoList, XT_OVER, XT_doLIT, CELLL, XT_U_2ER, XT_SPACE, XT__3ER, XT_branch, XT_dm_2B + 0x44, XT_DUP, XT_C_40, XT_doLIT, 3, XT_U_2ER, XT_doLIT, 1, XT__2B, XT_next, XT_dm_2B + 0x24, XT_EXIT,
...
/* 0x40002334: VER */ F_tokenNextVal, 0xc,
...
void TEST() { //  a1 a2 a3 b1 b2 b3 n -- ; Check n parameters on stack
  ...
  if ((ramSPP - ramSP) != byte(stackDepth << 1)) {
...
byte Mfetch8(const CELLTYPE byteAddr) {
  const byte offset = byteAddr & CELLOFFSETMASK;
#ifdef LITTLEENDIAN
  const byte shiftbits = offset << 3;
#else
  const byte shiftbits = (mask - offset) << 3;
#endif
...
void Mstore8(const CELLTYPE byteAddr, byte v) {
  //Serial.print("Mstore8 a="); Serial.print(byteAddr); Serial.print(" v="); Serial.println(v);
    const byte offset = byteAddr & CELLOFFSETMASK;
#ifdef LITTLEENDIAN
    const byte shiftbits = 8 * offset ;
#else
    const byte shiftbits = 8 * (CELLOFFSETMASK - offset);
#endif
...
CELLTYPE IPnext() {
  ...
  IP += CELLL;
  return v; }
CELLTYPE Ufetch(const byte userindex) { return ram[ramUP + userindex]; } // userindex is a cell index, not a byte index
void Ustore(const byte userindex, const CELLTYPE w) { ram[ramUP + userindex] = w; } // userindex is a cell index not a byte index
...
CELLTYPE na2xt(const CELLTYPE na) {
    return Mfetch(na - byte(2 << CELLSHIFT));
}
...
void TXbangS(CELLTYPE byteaddr, const byte len) {
...
void DOES() {
  Mstore(
    Mfetch(Ufetch(LASToffset) - byte(2 << CELLSHIFT) ) + CELLL, // field after tokenVar compile by CREATE
...
void cStore() { const CELLTYPE a = SPpop(); const byte v = SPpop(); Mstore8(a, v); } //C! c a -- , Store character
